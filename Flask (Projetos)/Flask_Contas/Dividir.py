...
def Buscar_Dividir(n):
    ...
    for row in cursor:
        ...
        cnxn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER=' + server + ';DATABASE=' + database + ';UID=' + username + ';PWD=' + password)
        cursor = cnxn.cursor()
        cursor.execute("UPDATE [dbo].[Flask_Contas] set [Resultado_Dividir] = ?", [D])
        cnxn.commit()
        json_string = """{"Conta": """ + str(row[0]) + """ / """ + str(row[1]) + """ {"Resultado": '""" + str(D) + """'}}"""
        print(json_string)
    return json_string

#Chamar a função
#Buscar_Dividir("5.00","8.00","0.625")
