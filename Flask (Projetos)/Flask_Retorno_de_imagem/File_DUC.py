...
def getfileDUC(n,a,m):
    cnxn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER=' + server + ';DATABASE=' + database + ';UID=' + username + ';PWD=' + password)
    cursor = cnxn.cursor()
    cursor.execute("select [DMRA_FicheiroDUC] from [dbo].[TBL_DMRAT] where DMRA_Emp_NIF ='" +  str(n) + "'AND DMRA_Ano ='" +  str(a) + "'AND DMRA_Mes ='" + str(m) + "'")
    for row in cursor:
        return row
    return False

#Chamar a função
#getfileDUC('502793481','2016','12')